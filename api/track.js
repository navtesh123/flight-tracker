...
module.exports = async function handler(req, res) {
  ...
  let apiUrl = `https://opensky-network.org/api/tracks/all?icao24=${encodeURIComponent(icao24)}&time=0`;
  const headers = {};
  ...
  // If credentials, build authenticated URL for proxies
  if (username && password) {
    apiUrl = `https://${encodeURIComponent(username)}:${encodeURIComponent(password)}@opensky-network.org/api/tracks/all?icao24=${encodeURIComponent(icao24)}&time=0`;
  }
